...
describe('PatientTransactionService', () => {
  ...
  describe('executePatientWithInfectionTransaction', () => {
    ...
    it('dovrebbe gestire il fallimento nella creazione dell\'evento infezione', async () => {
      ...
      ).rejects.toThrow(expect.stringContaining('Transazione parzialmente completata'));
      ...
      expect(mockNotificationService.error).toHaveBeenCalledWith(
        expect.stringContaining('Paziente creato ma evento infezione fallito'),
        { persistent: true }
      );
    });
  ...
  describe('validateTransactionData', () => {
    ...
    it('dovrebbe rifiutare dati infezione nulli', async () => {
      const validPatient = { nome: 'Mario', cognome: 'Rossi', data_ricovero: '2024-01-15' };
