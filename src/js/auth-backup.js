...
export function updateAuthUI(session) {
    ...
    if (session) {
        ...
        document.getElementById('logout-button').addEventListener('click', async () => {
            ...
            // Triggerpa callback manualmente per sicurezza
...
function createAuthModal() {
    // Server interno aziendale (NON localhost di sviluppo)
    const isInternalServer = window.location.hostname.includes('vgold') || 
                            window.location.hostname.includes('interno');
    
    // Server localhost di sviluppo (Google dovrebbe funzionare)
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content" style="background: var(--card-bg); border: none; box-shadow: var(--shadow-lg);">
                    ...
                    <div class="modal-body" role="main">
                        ...
                        ` : ''}
                        ${isLocalhost ? `
                            <div class="alert alert-success d-flex align-items-center mb-3" role="alert" aria-live="polite">
...
function setupModalEventListeners() {
    ...
    emailForm?.addEventListener('submit', async (e) => {
        ...
        if (!result.success) {
            ...
            // Riattiva auto-bypass per futuri accessi
...
function autoEnableLocalhostBypass() {
    ...
    // Non auto-attivare se l'utente ha fatto logout manualmente
