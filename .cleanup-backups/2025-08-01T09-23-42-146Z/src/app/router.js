...
async function fetchView(viewName) {
    ...
    if (!views[path]) {
        logger.error(`Vista HTML non trovata: ${viewName} (path cercato: ${path})`);
...
export async function renderView() {
    ...
    try {
        ...
        if (!viewContainer) {
            logger.error("Fatal: #view-container non trovato.");
            return;
        }
        ...
        logger.error('Errore durante il rendering della vista:', error);
        if(viewContainer) {
