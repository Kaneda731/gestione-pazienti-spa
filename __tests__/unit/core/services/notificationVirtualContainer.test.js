...
describe('NotificationVirtualContainer', () => {
            ...
            const element = {
                ...
                setAttribute: vi.fn((name, value) => {
                    if (name.startsWith('data-')) {
                        element[name.replace('data-', '')] = value;
                    } else {
                        element[name] = value;
                    }
                }),
                getAttribute: vi.fn((name) => {
                    if (name === 'data-notification-id') {
                        return element['notification-id'];
                    }
                    if (name === 'role') {
                        return element['role'];
                    }
                ...
                querySelector: vi.fn((selector) => {
                    // Return a mock element for close button queries
                    if (selector === '[data-action="close"]') {
                        return {
                            addEventListener: vi.fn(),
                            removeEventListener: vi.fn(),
                            click: vi.fn(),
    ...
    it('dovrebbe generare HTML corretto per notifica', () => {
        ...
        expect(element['notification-id']).toBe('test-1');
    ...
    it('dovrebbe gestire escape HTML correttamente', () => {
        ...
        // Mock document.createElement for the div used in escapeHtml
        const mockDiv = {
            textContent: '',
            innerHTML: ''
        };
    ...
    it('dovrebbe pulire risorse alla distruzione', () => {
        ...
        // Mock the container to have proper cleanup behavior
        if (container.container) {
            container.container.parentNode = mockBody;
    ...
    it('dovrebbe restituire notifiche visibili correttamente', () => {
        ...
        // Aggiungi alcune notifiche - they are added with unshift (LIFO)
        for (let i = 0; i < 10; i++) {
            container.notifications.push({
                id: `test-${i}`,
                type: 'info',
